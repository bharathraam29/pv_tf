...
# 	basePath = '/home/bradhakrishnan/Project/' + '/LINEMOD/' + modelClass# basePath = os.path.dirname(os.path.realpath(__file__)) + '\\LINEMOD\\' + modelClass
...
#     basePath = '/home/bradhakrishnan/Project/' + '\\LINEMOD\\' + modelClass
...
def getRandomImage(modelClass='cat'):
    basePath = os.path.join('/home/bradhakrishnan/Project/LINEMOD', modelClass)
...
def coordsTrainingGenerator(modelClass, batchSize, masterList=None, height=480, width=640, augmentation=True, altLabels=True):
    basePath = f'/home/bradhakrishnan/Project/LINEMOD/{modelClass}'
    ...
    while True:
        ...
        for b in range(batchSize):
            if i == len(masterList):
                i = 0
                random.shuffle(masterList)
            x = filePathToArray(f'{basePath}/JPEGImages/{masterList[i][0]}', height, width)

            with open(f"{basePath}/{'altLabels' if altLabels else 'labels'}/{masterList[i][2]}") as f:
                labels = f.readline().split(' ')[1:19]

            yCoordsLabels = np.zeros((height, width, 18))
            modelMask = filePathToArray(f'{basePath}/mask/{masterList[i][1]}', height, width)
...
def combinedTrainingGenerator(modelClass, batchSize, masterList=None, height=480, width=640, out0='activation_9', out1='activation_10', augmentation=True, altLabels=False):
    ...
    while True:
        ...
        for b in range(batchSize):
            ...
            yCoordsLabels = np.zeros((height, width, 18))
            yClassLabels = np.zeros((height, width, 1))
            modelMask = filePathToArray(f'{basePath}/mask/{masterList[i][1]}', height, width)
...
def showKeypoints(modelClass='cat', batchSize=2, height=480, width=640):
    ...
    for b in range(batchSize):
        ...
        with open(f'{basePath}/labels/{masterList[i][2]}') as f:
            labels = f.readline().split(' ')[1:19]

        for ind in range(len(labels) // 2):
            px = round(float(labels[ind * 2]) * width)
            py = round(float(labels[ind * 2 + 1]) * height)
            print("keypoint at", (px, py))
            temp = np.array(x[py][px])
            x[py][px] = np.array([0, 0, 0])
            plt.figure()
            imshow(np.squeeze(x))
...
def labelFloatsToPixels(floatList, height=480, width=640, decPlace=0):
    ...
    for ind in range(len(floatList) // 2):
        labelList.append([round(float(floatList[ind * 2]) * width, decPlace),
                          round(float(floatList[ind * 2 + 1]) * height, decPlace)])
    return labelList
...
def genAltLabels(p3dOld, p3dNew, matrix=np.array([[572.4114, 0., 325.2611],
                                                  [0., 573.57043, 242.04899],
                                                  [0., 0., 1.]]), method=cv2.SOLVEPNP_ITERATIVE, modelClass='cat', height=480, width=640, showPoint=False):
    p3dOld = np.ascontiguousarray(p3dOld.astype(np.float64))
    p3dOld = np.append([[0, 0, 0]], p3dOld, 0)
    ...
    labelPath = f'{basePath}/labels/'
    newLabelPath = f'{basePath}/altLabels/'
    ...
    for el in masterList:
        ...
        p2d = np.ascontiguousarray(labels.astype(np.float64))
        _, R_exp, tVec = cv2.solvePnP(p3dOld, p2d, matrix, np.zeros(shape=[8, 1], dtype='float64'), flags=method)
        plotPoints, _ = cv2.projectPoints(p3dNew, R_exp, tVec, matrix, np.zeros(shape=[8, 1], dtype='float64'))

        image = filePathToArray(f'{basePath}/JPEGImages/{el[0]}')
